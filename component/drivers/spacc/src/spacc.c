...
// set MAX_DDT_ENTRIES = 17 for mazimum 1MByte buffer processing
// set MAX_DDT_ENTRIES = 65 for testing full 4 mbyte flash
//#define MAX_DDT_ENTRIES 65
#define MAX_DDT_ENTRIES 17
...
{
    volatile uint32_t* firsts = (uint32_t*)src;
    volatile uint32_t* firstd = (uint32_t*)dest;
...
#define BOUNDARY 0x0000ffff
#define BOUNDARY_BITS 16

static void DdtInit(tSPACC_DDT *pDDT,  void * pData, int len)
{
	int index=0;

	unsigned int dataAddress = (unsigned int)pData;

	while (len > 0)
	{
		pDDT->data = (void*)dataAddress;
		pDDT->dataSz = len;
		

		if ( (((dataAddress & BOUNDARY) + len) >> BOUNDARY_BITS) != 0)
		{
			// We crossed a boundary, so adjust the length of this DDT
			pDDT->dataSz = (BOUNDARY+1) - (dataAddress & BOUNDARY);
			// adjust the remaining length
			// adjust the next DDT address
			dataAddress += pDDT->dataSz;
		}
		len = len - pDDT->dataSz;
		pDDT++;
		index++;
...
{
    int retCode = SPACC_OK;
    tSPACC_REG* baseAddr = SPACC;
    tSPACC_DDT srcDDT[MAX_DDT_ENTRIES] __attribute__((aligned(8)));
    tSPACC_DDT dstDDT[MAX_DDT_ENTRIES] __attribute__((aligned(8)));
    tSPACC_DDT* pSrcDDT;
    tSPACC_DDT* pDstDDT;
    ...
    {
        DdtInit(pSrcDDT, (void*)config->in, config->inSz + config->aadSz + config->icvSz);
        DdtInit(pDstDDT, (void*)config->out, config->inSz + config->aadSz + config->icvSz);
    }
    else
    {
...
static int spacc_cipher_write_context(tSPACC_CONFIG* config)
{
    ...
    SPACC_ASSERT(config->keyMode > SPACC_KEY_DUK);
    ...
    baseAddr->key_sz = (0x80000000)|(config->keyMode<<8)|(config->keySz);
        ...
        case SPACC_OP_AES:
            {
                ...
                config->out  = out;
                config->inSz  = inSz;
                retCode = spacc_packet_enqueue_ddt_ex(config);
            }
            break;

        default:
        	retCode = SPACC_INVALID_ARGUMENT;
    }
...
int ESAL_SPACC_HashEx(void* in, void* out, uint32_t inSz, uint32_t outSz, int mode, const uint8_t* hmacKey, uint32_t hmacKeySz, uint32_t ctxId)
{
    ...
    // HMAC key must always be provided.
	// HMAC key cannot come from SKP CTX.
