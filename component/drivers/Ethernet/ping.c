		...
		if(req>=5)
	   	{
	  		...
	  		UART_Printf("Ping Request = %d, PING_Reply = %d\r\n",req,rep);
	  	...
	  	len = recvfrom(s, (uint8_t *)data_buf,rlen,addr,&port);
			if(data_buf[0] == PING_REPLY) {
				...
				if(tmp_checksum != 0xffff)
					UART_Printf("tmp_checksum = %x\r\n",tmp_checksum);
				else{
					/*  Output the Destination IP and the size of the Ping Reply Message*/
				    	UART_Printf("Reply from %d.%d.%d.%d  ID:%x SeqNum:%x  :data size %d bytes\r\n",
				...
				PingReply.CheckSum = htons(checksum((uint8_t*)&PingReply,len));
					...
					/*  Output the Destination IP and the size of the Ping Reply Message*/
				    	UART_Printf("Request from %d.%d.%d.%d  ID:%x SeqNum:%x  :data size %d bytes\r\n",
