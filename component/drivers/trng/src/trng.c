...
int ESAL_TRNG_GetRandomData(void* out, uint32_t readSz, const void* reseed)
{
    ...
    int ret = TRNG_OK;
    int randLoop;
    uint8_t *pOutputBuffer = (uint8_t *)out;

    memset(pOutputBuffer, 0x0, readSz);
    ...
    if ( readSz <= TRNG_RANDOM_MAX_BYTE ) 
    {
        memcpy(pOutputBuffer, (void*)&baseAddr->rand_base, readSz);
        
        return (TRNG_OK);
    }
    
    randLoop = readSz / TRNG_RANDOM_MAX_BYTE;
    do {
        memcpy(pOutputBuffer, (void*)&baseAddr->rand_base, TRNG_RANDOM_MAX_BYTE);
        pOutputBuffer += TRNG_RANDOM_MAX_BYTE;
        trng_random_number_generate();
    } while ( --randLoop != 0 );
    
    if ( readSz & TRNG_RANDOM_REMIND_BYTE )
    {
        memcpy(pOutputBuffer, (void*)&baseAddr->rand_base, readSz & TRNG_RANDOM_REMIND_BYTE);
    }
...
int ESAL_TRNG_ReSeed(const void* nonce)
{
    ...
    if ( nonce )
    {
        smode = trng_rmw(&baseAddr->smode, 0xFFFFFFFF, 1UL << TRNG_SMODE_NONCE);
...
int ESAL_TRNG_GetSeed(void* out)
{
    ...
    {
        seedBuf[index] = baseAddr->seed_base[index];
    }

    memcpy((void*)out, (void*)seedBuf, 32);
