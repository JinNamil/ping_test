...
/**
 * @brief  Get ID of SPI3
 * @param  handle: pointer to a tSPI3_REG structure that contains the id information for SPI module
 * @param  data: data buffer with ID value of SPI
 * @retval if success, EWBMOK
 */
int SPI_EWBM_GetID(tSPI3_REG* reg, uint16_t* data)
{
    if ( reg == NULL )
        return EWBMINVARG;
