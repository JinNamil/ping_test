...
{
    if ( reg == NULL )
        return ((eSPI_INTR_MODE)EWBMINVARG);
 ...
 * @param  handle: pointer to a tSPI_REG structure that contains the id for SPI module
 * @param  data: data buffer with ID value of SPI
 * @retval  if success, EWBMOK
 */
int SPI_ARM_GetID(tSPI_REG* reg, uint16_t* data)
{
    int i;
 ...
 * @param  handle: pointer to a tSPI_REG structure that contains the cell id for SPI module
 * @param  data: data buffer with ID value of SPI
 * @retval  if success, EWBMOK
 */
int SPI_ARM_GetCellID(tSPI_REG* reg, uint16_t* data)
{
    int i;
