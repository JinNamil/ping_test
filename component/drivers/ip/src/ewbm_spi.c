...
static void spi_intr_cb(void *priv)
{
    ...
    if ( handle->bank == SPI_BANK_3 )
        mask = (eSPI_INTR_MODE)SPI_EWBM_IntrClear((tSPI3_REG*)handle->reg, mask);
    else
        mask = (eSPI_INTR_MODE)SPI_ARM_IntrClear((tSPI_REG*)handle->reg, mask);
}
...
/**
 * @brief  Get ID number of SPI
 * @param  handle: pointer to a tSPI_HANDLE structure that contains the id for SPI module
 * @param  data: data buffer with ID value of SPI
 * @retval if success, EWBMOK
 */
int SPI_GetID(tSPI_HANDLE* handle, uint16_t* data)
{
    int ret;
    ...
    if ( handle->bank == SPI_BANK_3 )
        ret = SPI_EWBM_GetID((tSPI3_REG*)handle->reg, data);
    else
        ret = SPI_ARM_GetID((tSPI_REG*)handle->reg, data);
    
    return ret;
}

/**
 * @brief  Get Cell ID number of SPI
 * @param  handle: pointer to a tSPI_HANDLE structure that contains the cell id for SPI module
 * @param  data: data buffer with Cell ID value of SPI
 * @retval if success, EWBMOK
 */
int SPI_GetCellID(tSPI_HANDLE* handle, uint16_t* data)
{
    int ret;
    ...
    ret = SPI_ARM_GetCellID((tSPI_REG*)handle->reg, data);
